...
const STATUS_MESSAGES = {
  not_ping: "Open an mtr.tools ping page to see statistics.",
...
function exportCsv() {
  ...
  download("mtrtools.csv", "text/csv", [header, ...lines].join("\n"));
}

function exportJson() {
  if (!latestData) return;
  download("mtrtools.json", "application/json", JSON.stringify(latestData.rows, null, 2));
}
